...
//        "yy年MM月dd日 HH:mm:ss";
    ...
    @Override
    public String formatDateTimeAsLanguageMode(String langIndex, String dateString, String direction) {

        String choosedUserDateFormat = AppConstant.YYYYMMDD_SLASH;

        if (langIndex.equals(AppConstant.LANG_INDEX_JAPANESE)) {
            choosedUserDateFormat = AppConstant.JAPANESE_DATE_FORMAT;
        }
...
//        dateString = dateString.substring(0, 8);
        try {
            if (direction.equals(AppConstant.FRONT_END_TO_BACK_END)) {
                ...
                if (langIndex.equals(AppConstant.LANG_INDEX_JAPANESE)) {
                    dateString = handleJapaneseYear(dateString);
                    dateString = formatJapaneseYearToEnglishYear(dateString);
                    DateTimeFormatterBuilder builder = AppConstant.DATE_TIME_FORMAT_OBJECT.get(AppConstant.JAPANESE_DATE_FORMAT_FROM_FRONTEND);
                    DateTimeFormatter formatter = builder.toFormatter();
                    LocalDateTime localDate = LocalDateTime.parse(dateString, formatter);
                    formatedDateTime = localDate.format(AppConstant.BACK_DATE_TIME_FORMATTER);

                } else if (choosedUserDateFormat.equals(AppConstant.YYYYMMDD)) {
                    dateString = handleEnglishYear(dateString);
                    DateTimeFormatterBuilder builder = AppConstant.DATE_TIME_FORMAT_OBJECT.get(AppConstant.YYYYMMDD);
                    DateTimeFormatter formatter = builder.toFormatter(Locale.US);
                    LocalDateTime localDate = LocalDateTime.parse(dateString, formatter);
                    formatedDateTime = localDate.format(AppConstant.BACK_DATE_TIME_FORMATTER);
...
//            DateTimeFormatter dateTimeFormatter = AppConstant.DATE_TIME_FORMAT_OBJECT.get(AppConstant.YYYYMMDD_HYPHEN).toFormatter();
                ...
                if (langIndex.equals(AppConstant.LANG_INDEX_JAPANESE)) {
                    dateString = formatEnglishYearToJapaneseYear(dateString);
                    LocalDateTime lcDate = LocalDateTime.parse(dateString, AppConstant.BACK_DATE_TIME_FORMATTER);
    ...
    @Override
    public String getTimeValidatation(String timeString, String language) {
        String current_time = getCurrentTime();
        String trimedTime = "";
        if (timeString != null) {
            trimedTime = getPlainTimeFormat(timeString);
        }

        String dateTime = current_time.substring(0, 8) + trimedTime + "00";

        String backendTime = formatDateTimeAsLanguageMode(language, dateTime, AppConstant.BACK_END_TO_FRONT_END);

        if (backendTime.equals("")) {
            return AppConstant.FALSE_VALUE;
        } else {
